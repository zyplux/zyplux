...
- VS Code Insiders' Electron does **not** silently override the render node; passing `--render-node-override` hangs the GPU process at startup. Hence `[code_insiders].switches = []` in `apps_config.toml`.
...
`--render-node-override=/dev/dri/renderD129` in `apps_config.toml [code_insiders].switches` made the GPU process never spawn (`coredumpctl list code-insiders` shows three SIGTRAPs that day). Removed; switches now `[]`. VS Code starts cleanly.
...
Stale `--render-node-override=/dev/dri/renderD129` removed from the `.desktop` Exec lines that `configure_apps.py` had previously written. Will need to re-run `sudo ./src/configure_apps.py` next time to confirm idempotence — repo state already matches the materialized state, so the script should be a no-op.
...
### 2026-05-19 — apps_config.toml symmetrization
