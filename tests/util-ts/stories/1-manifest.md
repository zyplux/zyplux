...
1. 1.2.3 normalizes an underscored requirement name into its pep 503 canonical form
2. 1.2.4 normalizes a dotted requirement name with a version specifier
3. 1.2.5 returns undefined when no package name can be parsed from a requirement

## 1.3 resolving a manifest's repository url

1. 1.3.1 reads the url from a string repository field
2. 1.3.2 reads the url from an object repository field
3. 1.3.3 returns undefined when no repository is declared
