...
describe('1.2 collecting and normalizing dependency names from a manifest', () => {
  ...
  test('1.2.3 normalizes a requirement name into its pep 503 canonical form', () => {
  ...
  test('1.2.4 returns undefined when no package name can be parsed from a requirement', () => {
    expect(normalizePythonName('\t \n')).toBeUndefined();
  });
});

describe("1.3 resolving a manifest's repository url", () => {
  test('1.3.1 reads the url from a string repository field', () => {
