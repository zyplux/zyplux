...
describe('9.1 flagging consumed push and unshift return values', () => {
  test('9.1.1 flags the push or unshift length assigned to a variable', ({ lintRule }) => {
    expect(lintRule('declare const items: number[]; const length = items.push(1);')).toReport('noReturnArrayPush');
    expect(lintRule('declare const items: number[]; const length = items.unshift(1);')).toReport('noReturnArrayPush');
  });

  test('9.1.2 flags a push result passed as an argument or consumed by the void operator', ({ lintRule }) => {
    expect(lintRule('declare const items: number[]; console.log(items.push(1));')).toReport('noReturnArrayPush');
    expect(lintRule('declare const items: number[]; void items.push(1);')).toReport('noReturnArrayPush');
  });

  test('9.1.3 flags awaiting an array push, unlike a promise-returning git push', ({ lintRule }) => {
    expect(lintRule('declare const items: number[]; async function run() { await items.push(1); }')).toReport(
      'noReturnArrayPush',
    );
  });

  test('9.1.4 flags a push result used as a logical operand', ({ lintRule }) => {
    expect(lintRule('declare const items: number[]; declare const ready: boolean; ready && items.push(1);')).toReport(
      'noReturnArrayPush',
    );
  });
  ...
  test('9.1.6 flags a union-element array receiver and a cast wrapping the call', ({ lintRule }) => {
    expect(lintRule('declare const items: (number | string)[]; const length = items.push("a");')).toReport(
      'noReturnArrayPush',
    );
    expect(lintRule('declare const items: number[]; const length = items.push(1) as number;')).toReport(
      'noReturnArrayPush',
    );
  });
...
describe('9.2 permitting discarded pushes and non-array receivers', () => {
  test('9.2.1 allows push and unshift as their own statements', ({ lintRule }) => {
    expect(lintRule('declare const items: number[]; items.push(1);')).toReportNothing();
    expect(lintRule('declare const items: number[]; items.unshift(1);')).toReportNothing();
  });

  test('9.2.2 allows an optional-chained push statement and a cast around a bare push', ({ lintRule }) => {
    expect(lintRule('declare const items: number[] | undefined; items?.push(1);')).toReportNothing();
    expect(lintRule('declare const items: number[]; items.push(1) as unknown;')).toReportNothing();
  });
  ...
  test('9.2.4 allows an argument-less push, which is a length read rather than an append', ({ lintRule }) => {
    expect(lintRule('declare const items: number[]; const length = items.push();')).toReportNothing();
  });

  test('9.2.5 leaves computed member access, free-standing push functions, and any receivers alone', ({ lintRule }) => {
    expect(lintRule('declare const items: number[]; const length = items["push"](1);')).toReportNothing();
    expect(lintRule('declare function push(value: number): number; const length = push(1);')).toReportNothing();
    expect(lintRule('declare const bag: any; const length = bag.push(1);')).toReportNothing();
  });
});
