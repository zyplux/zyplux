...
describe('8.1 rewriting property-only parameters into destructuring patterns', () => {
  ...
  test.for(cases)('8.1.%s', ([, pairs, expectLintReport], { fixRule, lintRule }) => {
...
describe('8.2 reporting without an autofix when destructuring would collide', () => {
  ...
  test.for(cases)('8.2.%s', ([, codes], { expectEachToReport, lintRule }) => {
...
describe('8.3 permitting parameters that need their whole object', () => {
  const cases: LintCase[] = [
    ['1 leaves an untyped parameter alone', ['const inferred = (node) => node.parent;']],
    [
  ...
  test.for(cases)('8.3.%s', ([, codes], { expectEachToReportNothing, lintRule }) => {
