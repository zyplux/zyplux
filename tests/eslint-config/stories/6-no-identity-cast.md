...
1. flags an expression-bodied identity with a typed parameter
2. flags an identity that also annotates the return position
3. flags a block-bodied identity with a single return
...
2. allows an untyped parameter, which asserts no type
