...
describe('4. restricting parent-relative (../) imports', () => {
  describe('4.1 flagging parent-relative import specifiers', () => {
    const cases: ImportCase[] = [
      ['1 flags a parent import at any depth', ["import { x } from '../x';", "import { x } from '../../../x';"]],
      ['2 flags a type only import from a parent path', ["import type { X } from '../x';"]],
      ['3 flags a re export from a parent path', ["export { x } from '../x';"]],
    ];

    test.for(cases)('4.1.%s', ([, codes], { lint }) => {
  ...
  describe('4.2 permitting non-parent-relative import specifiers', () => {
    const cases: ImportCase[] = [
      ['1 allows a same directory relative import', ["import { x } from './x';"]],
      ['2 allows alias and bare package imports', ["import { x } from '@/x';", "import { x } from 'node:path';"]],
    ];

    test.for(cases)('4.2.%s', ([, codes], { lint }) => {
