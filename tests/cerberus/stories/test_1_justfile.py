...
INSTALL_RECIPE = "# Install both workspaces: bun + uv.\ninstall:\n    bun install\n    uv sync --all-packages --all-groups\n\n"
...
WRONG_CHECK_ORDER = CONFORMING.replace("check: install knip typecheck lint test", "check: install lint knip typecheck test")
INTERLEAVED_CHECK = CONFORMING.replace("check: install knip typecheck lint test", "check: install knip build typecheck lint test") + (
    "\nbuild:\n    bun run build\n"
)
...
CLEAN_RUNNER_WRAPS_UNRELATED_COMMAND = CONFORMING.replace("    bun run cz clean {{ flags }}\n", "    bun run echo cz clean\n")
...
def test_1_1_2_fails_when_the_repo_has_no_justfile_at_its_root(run_justfile_check: RunJustfileCheck, status: type[Status]) -> None:
...
@requires_just
def test_1_1_3_errors_when_the_justfile_cannot_be_parsed(run_justfile_check: RunJustfileCheck, status: type[Status]) -> None:
...
@requires_just
def test_1_2_2_fails_when_a_required_recipe_is_missing(run_justfile_check: RunJustfileCheck, status: type[Status]) -> None:
...
@requires_just
def test_1_2_3_fails_when_a_recommended_alias_or_recipe_is_missing(run_justfile_check: RunJustfileCheck, status: type[Status]) -> None:
...
@requires_just
def test_1_3_1_fails_when_the_check_recipe_runs_its_steps_out_of_order(run_justfile_check: RunJustfileCheck, status: type[Status]) -> None:
...
@requires_just
def test_1_4_1_fails_when_the_default_recipe_does_not_list_available_commands(run_justfile_check: RunJustfileCheck, status: type[Status]) -> None:
...
@requires_just
def test_1_5_1_fails_and_names_the_tool_when_a_recipe_calls_it_directly(run_justfile_check: RunJustfileCheck, status: type[Status]) -> None:
...
@requires_just
def test_1_6_1_fails_when_a_recipe_line_has_trailing_whitespace(run_justfile_check: RunJustfileCheck, status: type[Status]) -> None:
...
@requires_just
def test_1_6_2_strips_trailing_whitespace_when_run_with_fix(run_check_on_disk: RunCheckOnDisk, tmp_path: Path, status: type[Status]) -> None:
...
@requires_just
def test_1_7_1_passes_a_conforming_justfile_whose_recipes_use_interpolation(run_justfile_check: RunJustfileCheck, status: type[Status]) -> None:
    result = run_justfile_check(WITH_INTERPOLATION)
    assert (result.status, result.problems) == (status.PASS, [])


@requires_just
def test_1_8_1_passes_a_conforming_justfile_that_declares_modules(run_justfile_check: RunJustfileCheck, status: type[Status]) -> None:
    result = run_justfile_check(WITH_MODULES)
    assert (result.status, result.problems) == (status.PASS, [])


@requires_just
def test_1_8_2_errors_instead_of_crashing_on_a_module_with_a_degenerate_path(run_justfile_check: RunJustfileCheck, status: type[Status]) -> None:
...
@requires_just
def test_1_9_1_fails_when_no_recipe_in_the_check_pipeline_runs_cerberus(run_justfile_check: RunJustfileCheck, status: type[Status]) -> None:
...
@requires_just
def test_1_9_3_does_not_count_a_mere_mention_of_cerberus(run_justfile_check: RunJustfileCheck, status: type[Status]) -> None:
...
def test_1_9_4_counts_runner_wrapped_cerberus_invocations(run_justfile_check: RunJustfileCheck, invocation: str) -> None:
    result = run_justfile_check(CONFORMING.replace("uv run cerberus --fix", invocation))
    assert structural_messages(result) == []


@requires_just
def test_1_10_1_fails_when_the_baseline_markers_are_missing(run_justfile_check: RunJustfileCheck, status: type[Status]) -> None:
...
@requires_just
def test_1_10_2_fails_naming_the_first_line_that_drifts_from_the_canonical_baseline(run_justfile_check: RunJustfileCheck, status: type[Status]) -> None:
...
@requires_just
@pytest.mark.parametrize("drifted_justfile", [DRIFTED_WITH_TAIL, MARKERS_WITH_TRAILING_WS], ids=["body-drift", "marker-trailing-ws"])
...
@requires_just
def test_1_10_4_refuses_to_fix_a_baseline_whose_rewrite_does_not_parse(run_check_on_disk: RunCheckOnDisk, tmp_path: Path, status: type[Status]) -> None:
...
@requires_just
def test_1_11_1_fails_when_the_clean_recipe_does_not_invoke_cz_clean(run_justfile_check: RunJustfileCheck, status: type[Status]) -> None:
...
@requires_just
def test_1_11_4_does_not_count_a_mere_mention_of_cz_clean(run_justfile_check: RunJustfileCheck, status: type[Status]) -> None:
...
@requires_just
def test_1_11_5_does_not_count_a_runner_wrapping_an_unrelated_command(run_justfile_check: RunJustfileCheck, status: type[Status]) -> None:
