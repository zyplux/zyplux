...
def test_18_1_1_skips_repos_with_no_pyproject_file(run_pytest_coverage: RunPytestCoverage, finding: type[Finding], status: type[Status]) -> None:
    result = run_pytest_coverage(None)

    assert result.findings == [finding(status.SKIP, "no pyproject.toml (not a Python repo)")]


def test_18_2_1_errors_when_pyproject_cannot_be_parsed(run_pytest_coverage: RunPytestCoverage, finding: type[Finding], status: type[Status]) -> None:
...
def test_18_2_3_fails_when_fail_under_is_not_a_number(run_pytest_coverage: RunPytestCoverage, finding: type[Finding], status: type[Status]) -> None:
    pyproject = "[project]\nname = 'demo'\n\n[tool.coverage.report]\nfail_under = 'ninety'\n"

    result = run_pytest_coverage(pyproject)

    assert result.findings == [finding(status.FAIL, "pyproject.toml [tool.coverage.report] fail_under must be a number; found 'ninety'")]


def test_18_3_1_fails_when_fail_under_is_below_the_required_floor(run_pytest_coverage: RunPytestCoverage, finding: type[Finding], status: type[Status]) -> None:
    pyproject = "[project]\nname = 'demo'\n\n[tool.coverage.report]\nfail_under = 80\n"

    result = run_pytest_coverage(pyproject)

    assert result.findings == [finding(status.FAIL, "pyproject.toml [tool.coverage.report] fail_under is 80, below the required 90")]
