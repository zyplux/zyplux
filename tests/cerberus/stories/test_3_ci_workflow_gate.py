...
def test_3_1_1_fails_when_no_workflow_file_exists(run_ci_workflow: RunCiWorkflow, fail: MakeFinding) -> None:
    result = run_ci_workflow(None)
    assert result.findings == [fail("no .github/workflows/ci.yml")]
...
def test_3_1_3_errors_when_the_workflow_is_not_a_mapping(run_ci_workflow: RunCiWorkflow, error: MakeFinding) -> None:
    result = run_ci_workflow("- just\n- a\n- list\n")
    assert result.findings == [error("ci.yml did not parse to a mapping")]
...
def test_3_1_4_passes_when_the_workflow_lives_at_the_yaml_extension(
    ...
    assert result.findings == [ok("ci workflow present and wired")]


def test_3_2_1_fails_without_a_job_named_ci(run_ci_workflow: RunCiWorkflow, fail: MakeFinding) -> None:
    result = run_ci_workflow("on: [pull_request, push]\njobs:\n  build:\n    name: build\n")
    assert result.findings == [fail("no job named `ci` (the required status-check context)")]


def test_3_2_2_passes_when_a_job_id_is_named_ci(run_ci_workflow: RunCiWorkflow, ok: MakeFinding) -> None:
    result = run_ci_workflow("on: [pull_request, push]\njobs:\n  ci:\n    runs-on: x\n")
    assert result.findings == [ok("ci workflow present and wired")]


def test_3_2_3_passes_when_a_job_name_field_is_ci(run_ci_workflow: RunCiWorkflow, ok: MakeFinding) -> None:
    result = run_ci_workflow("on: pull_request\njobs:\n  build:\n    name: ci\n")
    assert result.findings == [ok("ci workflow present and wired")]


def test_3_3_1_fails_without_a_pull_request_trigger(run_ci_workflow: RunCiWorkflow, fail: MakeFinding) -> None:
    result = run_ci_workflow("on: push\njobs:\n  ci:\n    name: ci\n")
    assert result.findings == [fail("ci.yml does not trigger on pull_request")]
...
def test_3_3_2_passes_with_a_pull_request_or_pull_request_target_trigger(
    run_ci_workflow: RunCiWorkflow, trigger: str, ok: MakeFinding
) -> None:
    result = run_ci_workflow(f"on: {trigger}\njobs:\n  ci:\n    name: ci\n")
    assert result.findings == [ok("ci workflow present and wired")]


def test_3_3_3_passes_when_the_on_key_parses_to_a_boolean(run_ci_workflow: RunCiWorkflow, ok: MakeFinding) -> None:
