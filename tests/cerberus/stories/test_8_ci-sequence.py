...
_SEQUENCE_PASS = Finding(Status.PASS, "ci.yml runs the canonical sequence")
...
def test_8_2_2_errors_when_the_ci_workflow_is_not_a_valid_yaml_mapping(run_ci_sequence: RunCiSequence, ci: str) -> None:
...
def test_8_3_2_fails_when_a_required_python_step_is_missing_or_does_not_match_its_required_command(
    run_ci_sequence: RunCiSequence, ci: str, missing_step: str
) -> None:
    result = run_ci_sequence(python=True, ci=ci)
