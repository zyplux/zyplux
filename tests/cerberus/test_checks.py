...
FileReader = Callable[[Repo, str], str | None]  # a mock of ctx.file(repo, path)
...
@requires_just
def test_conforming_justfile_passes(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", lambda *_: CONFORMING)
    assert justfile_check.run(repo, ctx).status is Status.PASS


def test_missing_justfile_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", lambda *_: None)
    assert justfile_check.run(repo, ctx).status is Status.FAIL


@requires_just
def test_missing_required_alias_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", lambda *_: MISSING_REQUIRED_ALIAS)
    assert justfile_check.run(repo, ctx).status is Status.FAIL


@requires_just
def test_missing_recommended_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", lambda *_: MISSING_RECOMMENDED)
    assert justfile_check.run(repo, ctx).status is Status.FAIL


@requires_just
def test_wrong_check_pipeline_order_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", lambda *_: WRONG_CHECK_ORDER)
    assert justfile_check.run(repo, ctx).status is Status.FAIL


@requires_just
def test_bare_managed_tool_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
...
@requires_just
def test_trailing_whitespace_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", lambda *_: TRAILING_WHITESPACE)
    assert justfile_check.run(repo, ctx).status is Status.FAIL


def test_codeowners_missing_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", lambda *_: None)
    assert codeowners_check.run(repo, ctx).status is Status.FAIL


def test_codeowners_covers_github(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", lambda *_: "/.github/ @zyplux/admins\n")
    assert codeowners_check.run(repo, ctx).status is Status.PASS


def test_codeowners_wildcard_covers_github(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", lambda *_: "* @zyplux/admins\n")
    assert codeowners_check.run(repo, ctx).status is Status.PASS


def test_codeowners_lookalike_path_does_not_cover_github(
...
def _ci_workflow(content: str) -> FileReader:
    return lambda _r, p: content if p.endswith((".yml", ".yaml")) else None


def test_ci_workflow_missing_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", lambda *_: None)
    assert ci_workflow_check.run(repo, ctx).status is Status.FAIL


def test_ci_workflow_invalid_yaml_errors(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", _ci_workflow("a: [unterminated"))
    assert ci_workflow_check.run(repo, ctx).status is Status.ERROR


def test_ci_workflow_non_mapping_errors(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", _ci_workflow("- just\n- a\n- list\n"))
    assert ci_workflow_check.run(repo, ctx).status is Status.ERROR


def test_ci_workflow_missing_ci_job_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", _ci_workflow("on: [pull_request, push]\njobs:\n  build:\n    name: build\n"))
    assert ci_workflow_check.run(repo, ctx).status is Status.FAIL


def test_ci_workflow_missing_pull_request_fails(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
...
def test_ci_workflow_pull_request_only_passes(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
...
def test_ci_workflow_on_as_pyyaml_bool_key(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    # Bare `on:` parses to the Python key True, not the string "on".
    monkeypatch.setattr(ctx, "file", _ci_workflow("on:\n  pull_request:\n  push:\njobs:\n  ci:\n    name: ci\n"))
    assert ci_workflow_check.run(repo, ctx).status is Status.PASS


def test_ci_workflow_job_id_named_ci_passes(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", _ci_workflow("on: [pull_request, push]\njobs:\n  ci:\n    runs-on: x\n"))
    assert ci_workflow_check.run(repo, ctx).status is Status.PASS
...
def test_workflow_tooling_passes_on_workspace_toolchain(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
    monkeypatch.setattr(ctx, "workflows", lambda *_: {"ci.yml": _CLEAN_WORKFLOW})
    assert workflow_tooling_check.run(repo, ctx).status is Status.PASS


def test_workflow_tooling_skips_without_workflows(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
    monkeypatch.setattr(ctx, "workflows", lambda *_: {})
    assert workflow_tooling_check.run(repo, ctx).status is Status.SKIP


def test_workflow_tooling_flags_setup_node(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    wf = "jobs:\n  ci:\n    steps:\n      - uses: actions/setup-node@v4\n"
    monkeypatch.setattr(ctx, "workflows", lambda *_: {"ci.yml": wf})
    assert workflow_tooling_check.run(repo, ctx).status is Status.FAIL


def test_workflow_tooling_flags_install_action(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
...
def test_workflow_tooling_flags_apt_install(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    wf = "jobs:\n  ci:\n    steps:\n      - run: sudo apt-get install -y just\n"
    monkeypatch.setattr(ctx, "workflows", lambda *_: {"ci.yml": wf})
    assert workflow_tooling_check.run(repo, ctx).status is Status.FAIL


def test_workflow_tooling_allows_npm_publish(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
...
def test_rumdl_canonical_passes(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", lambda *_: _RUMDL_CANONICAL)
    assert rumdl_config_check.run(repo, ctx).status is Status.PASS


def test_rumdl_exclude_is_allowed(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    content = _RUMDL_CANONICAL.replace("]\n", ']\nexclude = ["reference_clones"]\n', 1)
    monkeypatch.setattr(ctx, "file", lambda *_: content)
    assert rumdl_config_check.run(repo, ctx).status is Status.PASS


def test_rumdl_old_config_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", lambda *_: _RUMDL_OLD)
    assert rumdl_config_check.run(repo, ctx).status is Status.FAIL


def test_rumdl_missing_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", lambda *_: None)
    assert rumdl_config_check.run(repo, ctx).status is Status.FAIL
...
def _pyrefly_ctx(
    ...
    *,
    ...
    monkeypatch.setattr(ctx, "paths", lambda *_: _PY_PATHS if paths is None else paths)


def test_pyrefly_strict_passes(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    _pyrefly_ctx(monkeypatch, ctx)
    assert pyrefly_config_check.run(repo, ctx).status is Status.PASS


def test_pyrefly_skips_non_python(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", lambda *_: None)
    assert pyrefly_config_check.run(repo, ctx).status is Status.SKIP


def test_pyrefly_skips_repo_without_python_source(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
...
def test_pyrefly_missing_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    _pyrefly_ctx(monkeypatch, ctx, pyrefly=None)
    assert pyrefly_config_check.run(repo, ctx).status is Status.FAIL


def test_pyrefly_wrong_preset_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    _pyrefly_ctx(monkeypatch, ctx, pyrefly='preset = "default"\n')
    assert pyrefly_config_check.run(repo, ctx).status is Status.FAIL


def test_pyrefly_invalid_toml_errors(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    _pyrefly_ctx(monkeypatch, ctx, pyrefly="preset = [unterminated\n")
    assert pyrefly_config_check.run(repo, ctx).status is Status.ERROR


def test_pyrefly_config_in_pyproject_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    _pyrefly_ctx(monkeypatch, ctx, pyproject='[tool.pyrefly]\npreset = "strict"\n')
    assert pyrefly_config_check.run(repo, ctx).status is Status.FAIL


def test_pyrefly_uncovered_production_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
...
def test_pyrefly_stray_top_level_error_kind_fails(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
...
def test_pyrefly_fully_strict_tests_pass(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    config = (
        'preset = "strict"\n'
        'project-includes = ["apps/cerberus/src", "tests/cerberus"]\n'
        'search-path = ["apps/cerberus/src"]\n'
...
def test_rumdl_fix_normalizes_and_preserves_exclude(tmp_path: Path) -> None:
...
def _wf(run_step: str) -> dict[str, str]:
    return {"ci.yml": f"jobs:\n  ci:\n    steps:\n      - run: {run_step}\n"}


def test_cerberus_step_present_passes(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "workflows", lambda *_: _wf("uv run cerberus"))
    assert cerberus_step_check.run(repo, ctx).status is Status.PASS


def test_cerberus_step_uvx_form_passes(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "workflows", lambda *_: _wf("uvx zyplux-cerberus"))
    assert cerberus_step_check.run(repo, ctx).status is Status.PASS


def test_cerberus_step_absent_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "workflows", lambda *_: _wf("bun run test"))
    assert cerberus_step_check.run(repo, ctx).status is Status.FAIL


def test_cerberus_step_no_workflows_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "workflows", lambda *_: {})
    assert cerberus_step_check.run(repo, ctx).status is Status.FAIL


def test_cerberus_step_broken_yaml_errors(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "workflows", lambda *_: {"ci.yml": "jobs: [unterminated"})
...
def _ci_files(*, python: bool = False, ts: bool = False, ci: str = "") -> FileReader:
...
def test_ci_sequence_skips_without_manifest(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", _ci_files(ci=_PY_CI))
    assert ci_sequence_check.run(repo, ctx).status is Status.SKIP


def test_ci_sequence_python_canonical_passes(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", _ci_files(python=True, ci=_PY_CI))
    assert ci_sequence_check.run(repo, ctx).status is Status.PASS


def test_ci_sequence_python_missing_step_fails(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
...
def test_ci_sequence_python_unlocked_sync_fails(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
...
def test_ci_sequence_python_out_of_order_fails(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
    ci = (
        "jobs:\n  ci:\n    steps:\n"
...
def test_ci_sequence_ts_in_container_passes(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    monkeypatch.setattr(ctx, "file", _ci_files(ts=True, ci=_TS_CI))
    assert ci_sequence_check.run(repo, ctx).status is Status.PASS


def test_ci_sequence_ts_without_container_fails(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
...
def test_ci_sequence_ts_missing_step_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    ci = _TS_CI.replace("      - run: bun run knip\n", "")
    monkeypatch.setattr(ctx, "file", _ci_files(ts=True, ci=ci))
    assert ci_sequence_check.run(repo, ctx).status is Status.FAIL


def test_ci_sequence_missing_ci_file_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
...
def _repo_files(mapping: dict[str, str]) -> FileReader:
    def lookup(_repo: Repo, path: str) -> str | None:
...
def _wire_files(monkeypatch: pytest.MonkeyPatch, ctx: context.Context, mapping: dict[str, str]) -> None:
    monkeypatch.setattr(ctx, "paths", lambda _repo: sorted(mapping))
    monkeypatch.setattr(ctx, "file", _repo_files(mapping))


def test_vitest_runner_skips_without_package_json(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
...
def test_vitest_runner_passes_on_vitest_repo(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
...
def test_vitest_runner_flags_bun_test_import(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
...
def test_vitest_runner_flags_bun_test_script(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    _wire_files(monkeypatch, ctx, {"package.json": _BUN_TEST_PKG, "src/a.test.ts": _VITEST_IMPORT})
    assert vitest_runner_check.run(repo, ctx).status is Status.FAIL


def test_vitest_runner_allows_bun_filter_script(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
    _wire_files(monkeypatch, ctx, {"package.json": _BUN_FILTER_PKG, "src/a.test.ts": _VITEST_IMPORT})
    assert vitest_runner_check.run(repo, ctx).status is Status.PASS


def test_vitest_runner_allows_bun_run_test_script(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
...
def test_vitest_runner_ignores_node_modules(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    _wire_files(
        monkeypatch,
        ctx,
...
def test_ts_project_references_skips_without_package_json(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
    _wire_files(monkeypatch, ctx, {"README.md": "# demo\n"})
...
def test_ts_project_references_skips_non_workspace(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
...
def test_ts_project_references_skips_without_tsconfig(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
...
def test_ts_project_references_passes_on_tsc_build(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
...
def test_ts_project_references_passes_on_tsc_build_long_flag(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
...
def test_ts_project_references_flags_bun_filter_fanout(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
...
def test_ts_project_references_flags_single_project(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
...
def test_ts_project_references_flags_missing_typecheck(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
...
def test_catalog_discipline_skips_non_workspace(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
...
def test_catalog_discipline_passes_when_all_pinned(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
    _wire_files(
        monkeypatch,
...
def test_catalog_discipline_flags_raw_version(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
    _wire_files(
        monkeypatch,
...
def test_catalog_discipline_ignores_node_modules(
    monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context
) -> None:
    _wire_files(
        monkeypatch,
...
_RUFF_120 = "line-length = 120\n"
_PRETTIER_120 = "const config = {\n  printWidth: 120,\n};\n"


def _line_length_ctx(monkeypatch: pytest.MonkeyPatch, ctx: context.Context, *, files: dict[str, str]) -> None:
    monkeypatch.setattr(ctx, "file", lambda _r, p: files.get(p))


def test_line_length_matching_passes(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    _line_length_ctx(monkeypatch, ctx, files={"ruff.toml": _RUFF_120, "prettier.config.ts": _PRETTIER_120})
    assert line_length_check.run(repo, ctx).status is Status.PASS


def test_line_length_ruff_mismatch_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    _line_length_ctx(
        monkeypatch,
        ctx,
        files={"ruff.toml": "line-length = 100\n", "prettier.config.ts": _PRETTIER_120},
    )
    assert line_length_check.run(repo, ctx).status is Status.FAIL


def test_line_length_prettier_mismatch_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    _line_length_ctx(
        monkeypatch,
        ctx,
        files={"ruff.toml": _RUFF_120, "prettier.config.ts": "export default { printWidth: 80 };\n"},
    )
    assert line_length_check.run(repo, ctx).status is Status.FAIL


def test_line_length_ruff_missing_key_fails(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    _line_length_ctx(monkeypatch, ctx, files={"ruff.toml": "[lint]\n", "prettier.config.ts": _PRETTIER_120})
    assert line_length_check.run(repo, ctx).status is Status.FAIL


def test_line_length_only_ruff_passes(monkeypatch: pytest.MonkeyPatch, repo: Repo, ctx: context.Context) -> None:
    _line_length_ctx(monkeypatch, ctx, files={"ruff.toml": _RUFF_120})
    assert line_length_check.run(repo, ctx).status is Status.PASS
